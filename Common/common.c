...
/*
 * 我自己写的高精度延迟函数，理论上可以最多可以延迟13927.2年
 */
#define SYSTICKCLOCK		SYSCLK/8

void MYdelay_init(void)
{
	SysTick->CTRL &= ~(1<<0);     /*第0位：ENABLE，Systick 使能位  		（0：关闭Systick功能；1：开启Systick功能）关闭中断*/
	SysTick->CTRL &= ~(1<<2);			/*第2位：CLKSOURCE，Systick时钟源选择	（0：使用HCLK/8 作为Systick时钟；1：使用HCLK作为Systick时钟）*/			
	SysTick->CTRL &= ~(1<<1);			/*第1位：TICKINT，Systick 中断使能位    （0：关闭Systick中断；1：开启Systick中断）*/				
	SysTick->LOAD  = 5000000;			/*Systick是一个递减的定时器，当定时器递减至0时，重载寄存器中的值就会被重装载，继续开始递减。STK_LOAD  重载寄存器是个24位的寄存器最大计数0xFFFFFF*/		
	SysTick->VAL	 = 0x00;        /*清空计数器，VAL是个24位的寄存器，读取时返回当前倒计数的值，写它则使之清零，同时还会清除在SysTick 控制及状态寄存器中的COUNTFLAG 标志（第16位）*/
	SysTick->CTRL |=  (1<<0);			/*启动systick,开始倒数*/	
}

void udelay(uint32_t t)
{
	volatile int prev ,current;
	prev = SysTick->VAL;
	volatile long long count = SYSTICKCLOCK*t;		/*systick的时钟周期时1/21us，即VAL每减一个数延时1/21us, 延时1us则需要递减21次，延时t us则需要t*21次*/
	
	while(count > 0)
	{
		current = SysTick->VAL;
		//printf("udelay count: %d cur:%d\r\n", count, current);
		if(current < prev)
		{
			count -= (prev - current);
		}
		else{
			count -= (5000000 - current + prev);
		}
		prev = current;
	}	
}
